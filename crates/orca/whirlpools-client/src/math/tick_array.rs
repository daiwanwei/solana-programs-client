...
use crate::{
    constants::{
        ...
        TICK_ARRAY_NOT_EVENLY_SPACED, TICK_ARRAY_SIZE, TICK_INDEX_OUT_OF_BOUNDS,
        TICK_SEQUENCE_EMPTY,
    },
    types::{TickArrayFacade, TickFacade},
};
...
impl<const SIZE: usize> TickArraySequence<SIZE> {
    ...
    pub fn start_index(&self) -> i32 { start_tick_index(&self.tick_arrays[0]).max(MIN_TICK_INDEX) }
    ...
    pub fn next_initialized_tick(
        ...
        loop {
            ...
            // If at the end of the sequence, we don't have tick info but can still return
            // the next tick index
    ...
    pub fn prev_initialized_tick(
        ...
        loop {
            // If at the start of the sequence, we don't have tick info but can still return
