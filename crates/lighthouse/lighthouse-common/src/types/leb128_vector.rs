...
use borsh::{io::Read, BorshDeserialize, BorshSerialize};
...
impl<T: BorshSerialize + BorshDeserialize> From<Vec<T>> for LEB128Vec<T> {
    fn from(vec: Vec<T>) -> Self { LEB128Vec(vec) }
}
...
mod tests {
    ...
    fn struct_serialize() {
        let leb128_vec: LEB128Vec<TestStruct> = LEB128Vec(vec![
            TestStruct { a: 1, b: 2, c: vec!["hello".to_string()] },
            TestStruct { a: 3, b: 4, c: vec!["world".to_string()] },
        ]);
    ...
    fn struct_deserialize() {
        ...
        TestStruct { a: 1, b: 2, c: vec!["hello".to_string()] }.serialize(&mut data).unwrap();

        TestStruct { a: 3, b: 4, c: vec!["world".to_string()] }.serialize(&mut data).unwrap();
