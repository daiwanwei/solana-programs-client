...
//! multiply-divide operations. This library uses U128 in place of u128 for u64
//! operations, and supports U128 operations.

use crate::libraries::big_num::{U128, U256, U512};

/// Trait for calculating `val * num / denom` with different rounding modes and
/// overflow protection.
///
/// Implementations of this trait have to ensure that even if the result of the
/// multiplication does not fit into the type, as long as it would fit after the
/// division the correct result has to be returned instead of `None`. `None`
/// only should be returned if the overall result does not fit into the type.
///
/// This specifically means that e.g. the `u64` implementation must, depending
/// on the arguments, be able to do 128 bit integer multiplication.
...
impl Upcast256 for U128 {
    fn as_u256(self) -> U256 { U256([self.0[0], self.0[1], 0, 0]) }
}
...
impl Downcast256 for U256 {
    fn as_u128(self) -> U128 { U128([self.0[0], self.0[1]]) }
}
...
impl Upcast512 for U256 {
    fn as_u512(self) -> U512 { U512([self.0[0], self.0[1], self.0[2], self.0[3], 0, 0, 0, 0]) }
}
...
impl Downcast512 for U512 {
    fn as_u256(self) -> U256 { U256([self.0[0], self.0[1], self.0[2], self.0[3]]) }
}
...
impl MulDiv for u64 {
    ...
    fn to_underflow_u64(self) -> u64 { self }
}
