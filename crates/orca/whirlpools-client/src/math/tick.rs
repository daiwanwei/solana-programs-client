...
use crate::{
    constants::{
        ...
        TICK_ARRAY_SIZE, TICK_INDEX_NOT_IN_ARRAY,
    },
    types::{TickRange, U128},
};
...
/// Get the first tick index in the tick array that contains the specified tick
/// index.
///
/// # Parameters
...
/// - `tick_index` - A i32 integer representing the tick integer
/// - `tick_spacing` - A i32 integer representing the tick spacing
/// - `round_up` - A boolean value indicating if the supplied tick index should
///   be rounded up. None will round to the nearest.
///
/// # Returns
...
pub fn get_full_range_tick_indexes(tick_spacing: u16) -> TickRange {
    ...
    TickRange { tick_lower_index: min_tick_index, tick_upper_index: max_tick_index }
}

/// Order tick indexes in ascending order.
/// If the lower tick index is greater than the upper tick index, the indexes
/// are swapped. This is useful for ensuring that the lower tick index is always
/// less than the upper tick index.
///
/// # Parameters
...
/// - `tick_array_start_index` - A i32 integer representing the start tick index
///   of the tick array
/// - `tick_spacing` - A u16 integer representing the tick spacing
///
/// # Returns
...
fn get_sqrt_price_negative_tick(tick: i32) -> u128 {
    ...
    let mut ratio: u128 =
        if abs_tick & 1 != 0 { 18445821805675392311 } else { 18446744073709551616 };
