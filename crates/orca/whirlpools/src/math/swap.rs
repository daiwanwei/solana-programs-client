...
impl AmountDeltaU64 {
    pub fn value(&self) -> u64 {
        match self {
            ...
            AmountDeltaU64::ExceedsMax => panic!("Amount exceeds u64::MAX"),
        }
    }

    pub fn exceeds_max(&self) -> bool { matches!(self, AmountDeltaU64::ExceedsMax) }
...
pub fn compute_swap(
    ...
    if amount_specified_is_input {
        ...
        .try_into()
        .map_err(|_| ErrorCode::NumberDownCastError)?;
    }
    ...
    let next_sqrt_price = if initial_amount_fixed_delta.lte(amount_calc) {
        ...
        .map_err(|_| ErrorCode::SqrtPriceX64Overflow)?
    };
    ...
    Ok(SwapStepComputation { amount_in, amount_out, next_price: next_sqrt_price, fee_amount })
}
...
fn get_amount_delta_a(
    ...
    let (sqrt_price_lower, sqrt_price_upper) = if sqrt_price_current < sqrt_price_target {
    ...
    let amount = if amount_specified_is_input {
        checked_mul_div_round_up(numerator, 1u128 << Q64_RESOLUTION, denominator)
            .map_err(|_| ErrorCode::MathOverflow)?
    } else {
        checked_mul_div(numerator, 1u128 << Q64_RESOLUTION, denominator)
            .map_err(|_| ErrorCode::MathOverflow)?
    };

    Ok(amount.try_into().map_err(|_| ErrorCode::NumberDownCastError)?)
}
...
fn get_amount_delta_b(
    ...
    let amount = if amount_specified_is_input {
        ...
        checked_mul_div(liquidity, sqrt_price_upper - sqrt_price_lower, 1u128 << Q64_RESOLUTION)
            .map_err(|_| ErrorCode::MathOverflow)?
    };
...
fn try_get_amount_delta_a(
    ...
    match amount.try_into() {
        Ok(value) => Ok(AmountDeltaU64::Valid(value)),
        Err(_) => Ok(AmountDeltaU64::ExceedsMax),
    }
}

fn try_get_amount_delta_b(
    sqrt_price_current: u128,
    sqrt_price_target: u128,
    liquidity: u128,
    amount_specified_is_input: bool,
