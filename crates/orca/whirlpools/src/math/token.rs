...
use crate::math::{bit_math::*, tick, u256_math::*};

#[derive(Debug, Error)]
pub enum ErrorCode {
    #[error("Multiplication operation resulted in an overflow")]
    MultiplicationOverflow,
    #[error("Multiplication and shift right operation resulted in an overflow")]
    MultiplicationShiftRightOverflow,
    #[error("Token amount exceeded maximum allowed value")]
    TokenMaxExceeded,
    #[error("Token amount subceeded minimum allowed value")]
    TokenMinSubceeded,
    #[error("Division by zero attempted")]
    DivideByZero,
    #[error("Square root price is out of bounds")]
...
pub fn try_get_amount_delta_a(
    ...
    match result {
        ...
        Err(_) => Ok(AmountDeltaU64::ExceedsMax(ErrorCode::TokenMaxExceeded)),
    }
}
...
pub fn get_next_sqrt_price_from_a_round_up(
    ...
    let price = div_round_up_if_u256(numerator, denominator, true)
        .map_err(|_| ErrorCode::TokenMaxExceeded)?;
    if price < tick::MIN_SQRT_PRICE_X64 {
        return Err(ErrorCode::TokenMinSubceeded);
    } else if price > tick::MAX_SQRT_PRICE_X64 {
