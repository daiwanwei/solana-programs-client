use crate::{
    constants::{
        CoreError, AMOUNT_EXCEEDS_MAX_U64, ARITHMETIC_OVERFLOW, INVALID_SQRT_PRICE_LIMIT_DIRECTION,
        MAX_SQRT_PRICE, MIN_SQRT_PRICE, SQRT_PRICE_LIMIT_OUT_OF_BOUNDS, ZERO_TRADABLE_AMOUNT,
    },
    math::{
        sqrt_price_to_tick_index, tick_index_to_sqrt_price, try_apply_swap_fee,
        try_apply_transfer_fee, try_get_amount_delta_a, try_get_amount_delta_b,
        try_get_max_amount_with_slippage_tolerance, try_get_min_amount_with_slippage_tolerance,
        try_get_next_sqrt_price_from_a, try_get_next_sqrt_price_from_b, try_reverse_apply_swap_fee,
        try_reverse_apply_transfer_fee, TickArraySequence,
    },
    types::{
        ExactInSwapQuote, ExactOutSwapQuote, TickArrays, TickFacade, TransferFee, WhirlpoolFacade,
    },
};
...
/// - `specified_token_a`: If `true`, the input token is token A. Otherwise, it
///   is token B.
...
pub fn swap_quote_by_input_token(
    ...
    let token_est_out =
        try_apply_transfer_fee(token_est_out_before_fee, transfer_fee_out.unwrap_or_default())?;
...
/// - `specified_token_a`: If `true`, the output token is token A. Otherwise, it
///   is token B.
/// - `slippage_tolerance`: The slippage tolerance in basis points.
/// - `whirlpool`: The whirlpool state.
/// - `tick_arrays`: The tick arrays needed for the swap.
...
/// Computes the amounts of tokens A and B based on the current Whirlpool state
/// and tick sequence.
///
/// # Arguments
/// - `token_amount`: The input or output amount specified for the swap. Must be
///   non-zero.
/// - `sqrt_price_limit`: The price limit for the swap represented as a square
///   root. If set to `0`, it defaults to the minimum or maximum sqrt price
///   based on the direction of the swap.
/// - `whirlpool`: The current state of the Whirlpool AMM, including liquidity,
///   price, and tick information.
/// - `tick_sequence`: A sequence of ticks used to determine price levels during
///   the swap process.
...
///
/// # Returns
/// A `Result` containing a `SwapResult` struct if the swap is successful, or an
/// `ErrorCode` if the computation fails. # Notes
...
pub fn compute_swap<const SIZE: usize>(
    ...
    while amount_remaining > 0 && sqrt_price_limit != current_sqrt_price {
        ...
        if specified_input {
            amount_remaining = amount_remaining
                ...
                .ok_or(ARITHMETIC_OVERFLOW)?;
            amount_calculated =
                amount_calculated.checked_add(step_quote.amount_out).ok_or(ARITHMETIC_OVERFLOW)?;
        } else {
            amount_remaining =
                amount_remaining.checked_sub(step_quote.amount_out).ok_or(ARITHMETIC_OVERFLOW)?;
    ...
    Ok(SwapResult { token_a, token_b, trade_fee })
}
...
fn compute_swap_step(
    ...
    Ok(SwapStepQuote { amount_in, amount_out, next_sqrt_price, fee_amount })
}
