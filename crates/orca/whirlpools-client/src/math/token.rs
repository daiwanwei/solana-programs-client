...
use crate::{
    constants::{
        CoreError, AMOUNT_EXCEEDS_MAX_U64, ARITHMETIC_OVERFLOW, BPS_DENOMINATOR,
        FEE_RATE_DENOMINATOR, INVALID_SLIPPAGE_TOLERANCE, INVALID_TRANSFER_FEE, MAX_SQRT_PRICE,
        MIN_SQRT_PRICE, SQRT_PRICE_OUT_OF_BOUNDS,
    },
    types::{TransferFee, U128},
};
...
pub fn try_get_amount_delta_b(
    ...
    let product: U256 =
        <U256>::from(liquidity).checked_mul(sqrt_price_diff.into()).ok_or(ARITHMETIC_OVERFLOW)?;
...
pub fn try_get_next_sqrt_price_from_a(
    ...
    let p =
        <U256>::from(current_sqrt_price).checked_mul(amount.into()).ok_or(ARITHMETIC_OVERFLOW)?;
    ...
    let current_liquidity_shifted =
        <U256>::from(current_liquidity).checked_shl(64).ok_or(ARITHMETIC_OVERFLOW)?;
    let denominator =
        if specified_input { current_liquidity_shifted + p } else { current_liquidity_shifted - p };
...
pub fn try_get_next_sqrt_price_from_b(
    ...
    let result =
        if specified_input { current_sqrt_price + delta } else { current_sqrt_price - delta };
...
pub fn try_apply_transfer_fee(amount: u64, transfer_fee: TransferFee) -> Result<u64, CoreError> {
    ...
    let numerator =
        <u128>::from(amount).checked_mul(transfer_fee.fee_bps.into()).ok_or(ARITHMETIC_OVERFLOW)?;
...
pub fn try_reverse_apply_transfer_fee(
    ...
    if transfer_fee.fee_bps > BPS_DENOMINATOR {
        ...
        let numerator =
            <u128>::from(amount).checked_mul(BPS_DENOMINATOR.into()).ok_or(ARITHMETIC_OVERFLOW)?;
        ...
        let fee_amount =
            raw_pre_fee_amount.checked_sub(amount.into()).ok_or(AMOUNT_EXCEEDS_MAX_U64)?;
...
/// e.g. Your estimated amount you send is 10000 with 100 slippage tolerance.
/// The max you send will be 10100.
///
/// # Parameters
/// - `amount`: The amount to apply the fee to
/// - `slippage_tolerance_bps`: The slippage tolerance in bps (should be in
///   range 0..BPS_DENOMINATOR)
///
/// # Returns
...
/// e.g. Your estimated amount you receive is 10000 with 100 slippage tolerance.
/// The min amount you receive will be 9900.
///
/// # Parameters
/// - `amount`: The amount to apply the fee to
/// - `slippage_tolerance_bps`: The slippage tolerance in bps (should be in
...
/// e.g. You received 9900 amount with 10000 fee rate. The fee amount will be
/// 100. So the amount before fee will be 10000.
///
/// # Parameters
/// - `amount`: The amount to reverse the fee from
